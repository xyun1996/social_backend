...
type HTTPHandler struct { chat *service.ChatService }

func NewHTTPHandler(chat *service.ChatService) *HTTPHandler { return &HTTPHandler{chat: chat} }
...
type governanceMemberRequest struct {
	ActorPlayerID  string `json:"actor_player_id"`
...
func (h *HTTPHandler) Routes() http.Handler {
	...
	mux.HandleFunc("GET /v1/conversations/{conversationID}/governance", h.handleGetConversationGovernance)
	mux.HandleFunc("POST /v1/conversations/{conversationID}/governance", h.handleUpdateConversationGovernance)
	mux.HandleFunc("POST /v1/conversations/{conversationID}/moderators", h.handleSetConversationModerator)
	mux.HandleFunc("POST /v1/conversations/{conversationID}/mutes", h.handleSetConversationMute)
...
func (h *HTTPHandler) handleHealth(w http.ResponseWriter, _ *http.Request) {
	transport.WriteJSON(w, http.StatusOK, transport.StatusPayload{Service: "chat", Status: "ok"})
}

func (h *HTTPHandler) handleCreateConversation(w http.ResponseWriter, r *http.Request) {
	var request createConversationRequest
	if err := json.NewDecoder(r.Body).Decode(&request); err != nil { transport.WriteError(w, invalidJSONError()); return }
	conversation, appErr := h.chat.CreateConversation(request.Kind, request.ResourceID, request.MemberPlayerIDs)
	if appErr != nil { transport.WriteError(w, *appErr); return }
...
func (h *HTTPHandler) handleListConversations(w http.ResponseWriter, r *http.Request) {
	...
	if appErr != nil { transport.WriteError(w, *appErr); return }
	transport.WriteJSON(w, http.StatusOK, map[string]any{"player_id": playerID, "count": len(conversations), "conversations": conversations})
}
...
func (h *HTTPHandler) handleListConversationSummaries(w http.ResponseWriter, r *http.Request) {
	...
	if appErr != nil { transport.WriteError(w, *appErr); return }
	transport.WriteJSON(w, http.StatusOK, map[string]any{"player_id": playerID, "count": len(summaries), "summaries": summaries})
}
...
func (h *HTTPHandler) handleReplayMessages(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil { transport.WriteError(w, invalidQueryError("after_seq must be an integer")); return }
	limit, err := parseIntQuery(r, "limit")
	if err != nil { transport.WriteError(w, invalidQueryError("limit must be an integer")); return }
	messages, appErr := h.chat.ReplayMessages(r.PathValue("conversationID"), playerID, afterSeq, limit)
	if appErr != nil { transport.WriteError(w, *appErr); return }
	transport.WriteJSON(w, http.StatusOK, map[string]any{"conversation_id": r.PathValue("conversationID"), "player_id": playerID, "after_seq": afterSeq, "count": len(messages), "messages": messages})
}
...
func (h *HTTPHandler) handleGetConversationGovernance(w http.ResponseWriter, r *http.Request) {
	descriptor, appErr := h.chat.GetConversationGovernance(r.PathValue("conversationID"))
	if appErr != nil { transport.WriteError(w, *appErr); return }
	transport.WriteJSON(w, http.StatusOK, descriptor)
}

func (h *HTTPHandler) handleUpdateConversationGovernance(w http.ResponseWriter, r *http.Request) {
	var request governanceUpdateRequest
	if err := json.NewDecoder(r.Body).Decode(&request); err != nil { transport.WriteError(w, invalidJSONError()); return }
	descriptor, appErr := h.chat.UpdateConversationGovernance(r.PathValue("conversationID"), request.ActorPlayerID, request.SendPolicy, request.VisibilityPolicy)
	if appErr != nil { transport.WriteError(w, *appErr); return }
	transport.WriteJSON(w, http.StatusOK, descriptor)
}

func (h *HTTPHandler) handleSetConversationModerator(w http.ResponseWriter, r *http.Request) {
	var request governanceMemberRequest
	if err := json.NewDecoder(r.Body).Decode(&request); err != nil { transport.WriteError(w, invalidJSONError()); return }
	descriptor, appErr := h.chat.SetConversationModerator(r.PathValue("conversationID"), request.ActorPlayerID, request.TargetPlayerID, request.Enabled)
	if appErr != nil { transport.WriteError(w, *appErr); return }
	transport.WriteJSON(w, http.StatusOK, descriptor)
}

func (h *HTTPHandler) handleSetConversationMute(w http.ResponseWriter, r *http.Request) {
	var request governanceMemberRequest
	if err := json.NewDecoder(r.Body).Decode(&request); err != nil { transport.WriteError(w, invalidJSONError()); return }
	descriptor, appErr := h.chat.SetConversationMute(r.PathValue("conversationID"), request.ActorPlayerID, request.TargetPlayerID, request.Enabled)
	if appErr != nil { transport.WriteError(w, *appErr); return }
	transport.WriteJSON(w, http.StatusOK, descriptor)
}
...
func (h *HTTPHandler) handleDeliveryPlan(w http.ResponseWriter, r *http.Request) {
	...
	if appErr != nil { transport.WriteError(w, *appErr); return }
	transport.WriteJSON(w, http.StatusOK, map[string]any{"conversation_id": r.PathValue("conversationID"), "sender_player_id": senderPlayerID, "count": len(targets), "targets": targets})
}

func (h *HTTPHandler) handleRecordOfflineDelivery(w http.ResponseWriter, r *http.Request) {
	var request offlineDeliveryRequest
	if err := json.NewDecoder(r.Body).Decode(&request); err != nil { transport.WriteError(w, invalidJSONError()); return }
	receipt, appErr := h.chat.RecordOfflineDelivery(map[string]any{"conversation_id": request.ConversationID, "message_id": request.MessageID, "recipient_player": request.RecipientPlayer, "delivery_mode": request.DeliveryMode})
	if appErr != nil { transport.WriteError(w, *appErr); return }
...
func invalidQueryError(message string) apperrors.Error { return apperrors.New("invalid_query", message, http.StatusBadRequest) }
func parseInt64Query(r *http.Request, key string) (int64, error) { raw := r.URL.Query().Get(key); if raw == "" { return 0, nil }; return strconv.ParseInt(raw, 10, 64) }
func parseIntQuery(r *http.Request, key string) (int, error) { raw := r.URL.Query().Get(key); if raw == "" { return 0, nil }; return strconv.Atoi(raw) }
